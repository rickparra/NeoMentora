package com.neomentora.neomentora.service;

import com.neomentora.neomentora.dao.SmallBusinessRepository;
import com.neomentora.neomentora.domain.SmallBusiness;
import com.neomentora.neomentora.dto.SmallBusinessDTO;
import com.neomentora.neomentora.exception.NotFoundException;
import com.neomentora.neomentora.exception.ResourceAlreadyExistsException;
import com.neomentora.neomentora.util.Pagination;
import org.springframework.data.domain.PageRequest;
import org.springframework.stereotype.Service;

import static com.neomentora.neomentora.util.Constant.EMAIL_ALREADY_REGISTERED;
import static com.neomentora.neomentora.util.Constant.USER_NOT_FOUND;

@Service
public class SmallBusinessService implements UserService {

    private final SmallBusinessRepository repository;

    public SmallBusinessService(SmallBusinessRepository repository) {
        this.repository = repository;
    }

    public SmallBusiness registerSmallBusiness(SmallBusinessDTO smallBusinessDTO) {
        repository
                .findByEmail(smallBusinessDTO.email())
                .ifPresent(it -> {
                    throw new ResourceAlreadyExistsException(EMAIL_ALREADY_REGISTERED);
                });


        return repository.save(new SmallBusiness(smallBusinessDTO));
    }

    public SmallBusiness getSmallBusinessById(Long id) {
        return repository.findById(id).orElseThrow(() -> new NotFoundException(USER_NOT_FOUND));
    }

    public Pagination<SmallBusiness> getAllSmallBusiness(Integer limit, Integer offset) {
        var pageRequest = PageRequest.of(offset, limit);
        var userPagination = repository.findAll(pageRequest);

        return new Pagination<>(userPagination);
    }

}
